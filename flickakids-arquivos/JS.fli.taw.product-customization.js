$(function () {
		...
		vtexjs.checkout.simulateShipping([{
			...
			if (attData.fields.illustrations != null) {
				...
				for (var i = 0; i < attData.fields.illustrations.length; i++) {
					...
					preLoad.append('<img src="/arquivos/qd-stamp-' + illustration.toLowerCase().replaceSpecialChars() + '.png" class="img-flag" />');
				}
			...
			if (attData.fields.colors != null) {
				...
				for (var i = 0; i < attData.fields.colors.length; i++) {
					...
					preLoad.append('<img src="/arquivos/personalizacao-cor-' + color.toLowerCase().replaceSpecialChars() + '.jpg" class="img-flag" />');
				}
			...
			modal.find('.qd-fpp-items .qd-illustration').on("change", function () {
				...
				if (vtxctx.categoryName.indexOf('Chupeta') > -1) {
	...
	function updateCustomize(text, imageUrl, font, preview) {
		...
		if (text && vtxctx.categoryName.indexOf('Chupeta') > -1) {
	...
	function validateForm(formArea, modal) {
		...
		$form.validate({
			...
			highlight: function (element, errorClass, validClass) {
				...
				if ($e.is(':radio'))
					$e.parents('.input-group-radio').addClass(errorClass).removeClass(validClass);
				else
					$e.parent().addClass(errorClass).removeClass(validClass);
			},
			unhighlight: function (element, errorClass, validClass) {
				...
				if ($e.is(':radio'))
					$e.parents('.input-group-radio').addClass(validClass).removeClass(errorClass);
				else
					$e.parent().addClass(validClass).removeClass(errorClass);
			},
	...
	function showSuccessModal(formArea, modal, attData) {
		...
		$(window).one('QuatroDigital.bia_callback', function () {
			...
			modalBody += '<h5 class="qd-fpp-success">Produto adicionado com sucesso!</h5>';
			modalBody += '<div class="product-qd-v1-buy-button"><a href="/checkout">Finalizar Compra</a></div>';
			modalBody += '<a href="javascript: void(0);" class="qd-fpp-continue" data-dismiss="modal">Continuar comprado</a>';
	...
	function prodCustomize(e) {
		...
		modalFooter += '<div class="product-qd-v1-buy-button"><a class="buy-button" href="#">Comprar personalizado</a></div>';
