...
try {
	var Common = {
		...
		applyCarouselShelf: function () {
			...
			wrapper.slick({
				slidesToShow: qqtProduct,
				slidesToScroll: qqtProduct,
				infinite: true,
				draggable: false,
				speed: 700,
				responsive: [
		...
		applyAmazingMenu: function () {
			...
			$('<span class="qd-am-dropdown-trigger"><i class="fas fa-chevron-down"></i></span>').prependTo($('.search-bar-qd-v1-toggle')).click(function () {
				var t = $(this);
				t.closest('ul').siblings('.header-qd-v1-search-wrap').stop().slideToggle();
				t.parent().toggleClass('qd-active');
			});
			
			wrapper.QD_amazingMenu({
				callback: function () {
					$('<span class="qd-am-dropdown-trigger"><i class="fas fa-chevron-down"></i></span>').prependTo(wrapper.find('.qd-am-has-ul'))
					.click(function () {
						var t = $(this);
						t.stop().siblings('ul.qd-am-dropdown').slideToggle();
						t.parent().toggleClass('qd-active');
			...
			$('.header-qd-v1-amazing-menu-trigger, .header-qd-v1-side-menu-toggle').click(function (evt) {
				evt.preventDefault();
			...
			$('.header-qd-v1-amazing-menu-mobile-close').click(function(){
				$(document.body).removeClass('qd-am-on');
			});
			
		},
		applySmartCart: function () {
			$('.header-qd-v1-cart, .fixed-buttons-qd-v1-cart-link').append('<div class="smart-cart-qd-v1-wrapper"><div class="qd-sc-wrapper"></div></div>');
			...
			$.QD_smartCart({
				...
				dropDown: {
					...
					smartCheckout: true,
					callback: function () {
						$(".qd-ddc-wrapper3").prepend('<div class="qd-cartTitle"><h3>Mi carrito</h3></div>');
		...
		formatProductName: function() {
			$('.shelf-qd-v1').each(function(){
				var productFullName = $(this).find('.shelf-qd-v1-name').text().trim().split('|');
				var brand = productFullName[0];
				var productName = productFullName[1];
				var manufacturerYear = productFullName[2];
				var years = productFullName[3];
				var observation = productFullName[4];
	...
	var Home = {
		...
		selectSmartResearch2: function () {
			var departmentId = 6350602; // /todos-los-productos 6350602
			var url, map = {};
			$.ajax({url: "/busca?lid=ad46bbf8-0f0e-4410-b625-42087df083ab&fq=C:/" + departmentId + "/", dataType: "html"}).done(function(data) {
				var qttRegex = /\s+\([0-9]+\)$/;
				var values = [];

				$(data).find(".search-single-navigator ul.Armadora").find("a").each(function() {
					var $t = $(this);
					values.push([$t.text().trim().replace(qttRegex, ""), $t.attr("href") || ""]);
				});
		
				$(".filter-qd-v1-wrapper .searchoptionswrapper").QD_SelectSmartResearch2({
					options: [values, "lid=ad46bbf8-0f0e-4410-b625-42087df083ab", "lid=ad46bbf8-0f0e-4410-b625-42087df083ab"],
					optionsPlaceHolder: ["Armadora", "Modelo", "Años"],
					disabledMessage: function(index, options, optionsPlaceHolder) {
						return "Seleccione " + optionsPlaceHolder[index] + "...";
					},
					...
					optionIsChecked: function(optionPlaceHolder) {
						...
						if(!url && location.search.toLowerCase().indexOf("map=") > -1){
							...
							map = {
								"Armadora": decodeURIComponent(urlMap.map.specificationFilter_47 || ""),
								"Modelo": decodeURIComponent(urlMap.map.specificationFilter_48 || ""),
								"Años": decodeURIComponent(urlMap.map.specificationFilter_46 || "")
							};
						}
	...
	var Search = {
		...
		addPageQttButtons: function() {
			$('.main .searchResultsTime').first().after('<div class="search-qd-v1-change-page"> <span>Mostrar artículos:</span> <ul> <li> <a href="PS=20" class="first">20</a> </li> <li> <a href="PS=35">35</a> </li> <li> <a href="PS=50" class="last">50</a> </li> </ul> </div>');

			$('.search-qd-v1-change-page a').click(function(e){
				e.preventDefault();
				...
				if (window.location.href.indexOf("?") > -1)
					queryString = window.location.search.replace(/PS=([^&]*)/g, $(this).attr('href'));				
		...
		addShelfDisplayButtons: function() {
			...
			$('.search-qd-v1-display-shelves a').click(function(){				
				var t = $(this);

				if (t.is('#display_list')) {
					$(document.body).addClass('busca-lista');
					$('.search-qd-v1-result').removeClass('qd-shelf-lg-4').addClass('qd-shelf-md-12');					
					t.closest('ul').find('a').removeClass('active');
					t.addClass('active');
				}
				else if (t.is('#display_grid')) {
					$(document.body).removeClass('busca-lista');
					$('.search-qd-v1-result').removeClass('qd-shelf-md-12').addClass('qd-shelf-lg-4');
					t.closest('ul').find('a').removeClass('active');
					t.addClass('active');
				}
			});
		...
		callInfinityScroll: function() {
			...
			wrapper.QD_infinityScroll({
				paginate: function(callback) {
					$('<button class="qd-ssb-more-products"><i class="fas fa-redo-alt"></i>SIGUIENTES PRODUCTOS</button>').insertAfter(wrapper).click(callback);
				},
				returnToTop: $("")
			});
	...
	var Product = {
		...
		scrollToDescription: function () {
			$('.product-qd-v1-description-link').click(function (e) {
				...
				$('html, body').stop().animate({
					'scrollTop': $('.product-qd-v1-description').offset().top - 175
				}, 900, 'swing');
			});
		},
		scrollToAvaliations: function () {
			$('.product-qd-v1-avaliations-link').click(function (e) {
				...
				$('html, body').stop().animate({
					'scrollTop': $('.product-qd-v1-avaliations').offset().top - 75
				}, 900, 'swing');
			});
		},
		...
		openShipping: function () {
			...
			$('.product-qd-v1-open-shipping').click(function (e) {
				var $t = $(this);
				$t.toggleClass('qd-is-active');
				$('.product-qd-v1-shipping-freight').toggleClass('qd-is-active');
			});
		},
		...
		applyAffixSideProduct: function () {
			...
			box.QD_affix({
				topSpacing: box.offset().top - 290,
				bottomSpacing: 0,
				bottomLimitSeletor: $('.product-qd-v1-avaliations'),
				css: {
					position: 'relative'
				}
			});
		...
		insertContentDiscount: function () {
			// $('<div class="product-qd-v1-discount">Ahorras: <strong>$149.16 (30%)</strong></div>').insertAfter('.price-list-price');
			...
			$(window).on('skuSelected.vtex', function (e, sku, data) {
				if (!discount.length)
				discount = $('<div class="product-qd-v1-discount">Ahorras: <strong>$0.00 <span>(0%)</span></strong></div>').insertAfter('.product-qd-v1-sku-selection-wrapper .price-list-price');
				
				if (data.listPrice > data.bestPrice) 
				discount.find('strong').html("$" + (data.listPrice - data.bestPrice) / 100 + "(" + parseInt(100 - data.bestPrice / data.listPrice * 100) + "%)").show();
				else
				discount.hide();
			});
			...
			if (skuJson.skus.length >= 1) {
				...
				if (skuJson.skus[0].listPrice > skuJson.skus[0].bestPrice)
				discount.find('strong').html("$" + (skuJson.skus[0].listPrice - skuJson.skus[0].bestPrice) / 100 + "<span>(" + parseInt(100 - skuJson.skus[0].bestPrice / skuJson.skus[0].listPrice * 100) + "%)</span>").show();
	...
	var Institutional = {
		...
		formContact: function () {
			if (!$(document.body).is(".contacto"))
			return;
			
			var jsnomeLoja = "devmastuning";
			...
			form.validate({
				...
				messages: {
					nombre: "Su nombre no es válido",
					correo: "Su e-mail no es válido."
				},
...
// qd-include web/dist/js/JS.jquery.validate.min.js
...
// qd-include ../qd-quatro-lib-plugins/Select Smart Research 2/QD_SelectSmartResearch2.min.js

// qd-include ../qd-quatro-lib-plugins/VTEX URL Parse/QD_VtexUrlParse.min.js