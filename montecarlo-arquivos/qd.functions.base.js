	...
	var Product = {
		...
		product360: function() {
			...
			window.ImageControl = function() {
				$.ajax({url: '//montecarlo.vteximg.com.br/arquivos/' + skuJson.productId + '_16.jpg'}).done(fn);
				ImageControl.apply(this, arguments);
			};

			$(window).on('skuSelected.vtex', function() {
				$('.y-productImg').removeClass('qd-v1-product-360-show');
				$('.qd-v1-product-360-image').remove();
				fn();
			});

			function fn() {
				var wrapper = $('.y-productImg').addClass('qd-v1-product-360');
				var thumbs = wrapper.find('.thumbs');

				if (wrapper.find('.qd-v1-product-360-image').length || thumbs.find('.qd-v1-product-360-thumb').length)
					return
				...
				thumbs.find('li:first').clone().appendTo(thumbs).addClass('qd-v1-product-360-thumb').click(function() {
					...
					product360 = $('.qd-v1-product-360-image .threesixty').ThreeSixty({
						...
						progress: '.spinner',
						imagePath:'http://montecarlo.vtexcommercestable.com.br/arquivos/' + skuJson.productId + '_',
						filePrefix: '',
						ext: '.jpg',
						drag: false,
						responsive: true,
						...
						onReady: function() {
							product360Image.mousemove(function(e){
								...
								x = Math.round((e.pageX - spaceLeft) * 16 	/ sizewidth);
							...
							slider.slider({
								...
								slide: function(event, ui) {
									product360.gotoAndPlay(ui.value);
								}
							});
						}
					});
...
/* * jQuery UI Touch Punch 0.2.2 * * Copyright 2011, Dave Furfero * Dual licensed under the MIT or GPL Version 2 licenses. * * Depends: *  jquery.ui.widget.js *  jquery.ui.mouse.js */
